...
class DbOperation
{
	...
	*/
	function createHero($code,$unlock_code,$uses_remaining,$amount,$start_date,$expire_date,$active,$radius){
		$stmt = $this->con->prepare("INSERT INTO rw_promo_code (code,unlock_code,uses_remaining,amount,start_date,expire_date,active,radius) VALUES ('$code','$unlock_code','$uses_remaining','$amount','$start_date','$expire_date','$active','$radius')");
	...
	*/
	...
	function getHeroes(){
		$stmt = $this->con->prepare("SELECT id, code,unlock_code,uses_remaining,amount,start_date,expire_date,active,radius FROM rw_promo_code");
		$stmt->execute();
		$stmt->bind_result($id, $code,$unlock_code,$uses_remaining,$amount,$start_date,$expire_date,$active,$radius);
	...
	function getActive($active){
		$stmt = $this->con->prepare("SELECT id, unlock_code,uses_remaining,amount,start_date,expire_date,active,radius FROM rw_promo_code WHERE active=$active");
		$stmt->execute();
		$stmt->bind_result($id, $unlock_code,$uses_remaining,$amount,$start_date,$expire_date,$active,$radius);
